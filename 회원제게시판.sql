...
EMPLOYEENM REFERENCES EMPLOYEE, 	-- 회원 이름
CREATE_DATE REFERENCES COM,			-- 작성일
SHOPPING_TITLE REFERENCES SHOPPING, -- 상품 제목
READ_COUNT REFERENCES SHOPPING,		-- 조회수
SHOPPING_CONTENT REFERENCES SHOPPING-- 상품 내용
);
...
SELECT EMPLOYEE_NO, EMPLOYEE_NM, SHOPPING_TITLE
FROM EMPLOYEE
NATURAL JOIN SHOPPING
WHERE SHOPPING_NO = '1';

SELECT ORDER_NO, EMPLOYEE_NM, SHOPPING_TITLE
FROM "ORDER"
NATURAL JOIN EMPLOYEE
NATURAL JOIN SHOPPING
WHERE SHOPPING_NO = '1';

SELECT EMPLOYEE_NM, SHOPPING_TITLE, ORDER_NO
FROM EMPLOYEE
NATURAL JOIN SHOPPING
NATURAL JOIN "ORDER"
WHERE SHOPPING_NO = '1';
...
MEM_NM VARCHAR2(30) NOT NULL, -- 이름
MEM_GENDER CHAR(1) CHECK(MEM_GENDER IN ('M', 'F')), -- 성별
ENROLL_DATE DATE DEFAULT SYSDATE, -- 가입일
SECESSION_FL CHAR(1) DEFAULT 'N' CHECK(SECESSION_FL IN ('Y', 'N')) -- 탈퇴여부
);
...
COMMENT ON COLUMN MEM.MEM_NM IS '회원 이름';
COMMENT ON COLUMN MEM.MEM_GENDER IS '회원 성별';
COMMENT ON COLUMN MEM.ENROLL_DATE IS '회원 가입일';
COMMENT ON COLUMN MEM.SECESSION_FL IS '탈퇴여부(Y/N)';
